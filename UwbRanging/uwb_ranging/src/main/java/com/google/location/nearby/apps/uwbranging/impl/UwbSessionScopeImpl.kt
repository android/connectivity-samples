...
internal abstract class UwbSessionScopeImpl(private val localEndpoint: UwbEndpoint) :
    UwbSessionScope {

    protected abstract fun startOob(): Flow<UwbOobEvent>
        ...
        val job = launch {
            startOob().collect { event ->
                when (event) {
                    ...
                    is UwbOobEvent.UwbEndpointLost -> endpointLost(event.endpoint)
                }
            }
        }
    ...
    private fun endpointLost(endpoint: UwbEndpoint) {
    ...
    private suspend fun ProducerScope< UwbRangingResult>.processEndpointFound(
        ...
        activeJobs[event.endpoint] =
            coroutineScope { launch { rangingEvents.collect { sendResult(it) } } }
    }
